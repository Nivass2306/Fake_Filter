...
def advanced_classification(text: str) -> dict:
    ...
    if model:
        try:
            ...
            prob_dict = {str(c).lower(): p for c, p in zip(model.classes_, prediction_prob)}
            ml_fake_prob = float(prob_dict.get("fake", prob_dict.get("1", 0.0)))
            ml_real_prob = float(prob_dict.get("real", prob_dict.get("0", 0.0)))
            ml_score = min(max(ml_fake_prob, 0.0), 1.0)
        except Exception as e:
    ...
    keyword_score = fake_score / (fake_score + reliable_score + 0.01)
    ...
    pattern_score = min(max(0.2 + suspicious_score - reliable_score * 0.5, 0.0), 1.0)
    ...
    fake_probability = round(final_fake_score * 100)
...
def get_detailed_explanation(classification_result, original_text):
    ...
    if classification_result["verdict"] == "Likely Fake":
        ...
        real_confidence = classification_result.get("realProbability", 100 - classification_result.get("score", 0))
        explanation_parts.append(f"✓ This content appears reliable ({real_confidence}% confidence).")
...
def analyze():
    ...
    result_data = {
        ...
        "fakeProbability": classification_result.get("fakeProbability"),
        "realProbability": classification_result.get("realProbability"),
